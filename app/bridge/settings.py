...
class SettingsController(QObject):
    ...
    @Property(int, notify=scale_changed)
    def scale(self) -> int:
        value = self._pref("scale")
        if isinstance(value, int) and value in SCALE_LEVELS:
            return value
        return PREF_DEFAULTS["scale"]
    ...
    def scale_levels(self) -> list:
        return list(SCALE_LEVELS)
    ...
    def set_scale(self, value: int) -> None:
        if value not in SCALE_LEVELS:
            self.settings_error.emit(
                "scale", f"Scale must be one of {', '.join(map(str, SCALE_LEVELS))}"
            )
            return
        if value == self.scale:
            return
        self._store_pref("scale", value)
