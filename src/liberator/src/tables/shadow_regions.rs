pub static SR_Y6S2_15701375: &[ShadowRegion] = &[
    ShadowRegion { offset: 34194451, patch: &[233, 218, 0, 0, 0] },
];
pub static SR_Y7S2_43489433: &[ShadowRegion] = &[
    ShadowRegion { offset: 70891833, patch: &[65, 177, 138, 144] },
    ShadowRegion { offset: 70891682, patch: &[233, 146, 0, 0, 0] },
    ShadowRegion { offset: 386998, patch: &[176, 0] },
    ShadowRegion { offset: 387014, patch: &[176, 0] },
    ShadowRegion { offset: 387035, patch: &[176, 0] },
];
pub static SR_Y7S4_50497889: &[ShadowRegion] = &[
    ShadowRegion { offset: 31939975, patch: &[179, 0, 235, 124] },
    ShadowRegion { offset: 28347181, patch: &[179, 0, 235, 123] },
    ShadowRegion { offset: 86200204, patch: &[144, 144, 144, 144, 233] },
    ShadowRegion { offset: 28554354, patch: &[179, 0, 144, 235, 122] },
    ShadowRegion { offset: 28385078, patch: &[179, 0, 144, 235, 122] },
    ShadowRegion { offset: 31990087, patch: &[179, 0, 235, 124] },
    ShadowRegion { offset: 32268965, patch: &[176, 0] },
    ShadowRegion { offset: 32268981, patch: &[176, 0] },
];
pub static SR_Y7S1_39785137: &[ShadowRegion] = &[
    ShadowRegion { offset: 38908722, patch: &[104] },
];
pub static SR_Y6S3_32627923: &[ShadowRegion] = &[
    ShadowRegion { offset: 62038739, patch: &[233, 113, 1, 0, 0, 144] },
];
pub static SR_Y6S4_37046065: &[ShadowRegion] = &[
    ShadowRegion { offset: 73741091, patch: &[233, 78, 1, 0, 0, 144] },
];
pub static SR_Y6S1_15500403: &[ShadowRegion] = &[
    ShadowRegion { offset: 73961721, patch: &[235] },
];
pub static SR_Y6S2_15772114: &[ShadowRegion] = &[
    ShadowRegion { offset: 4788275, patch: &[233, 115, 1, 0, 0, 144] },
];
pub static SR_Y6S3_35130040: &[ShadowRegion] = &[
    ShadowRegion { offset: 15866739, patch: &[233, 100, 1, 0, 0, 144] },
];
pub static SR_Y5S1_14303219: &[ShadowRegion] = &[
    ShadowRegion { offset: 27430755, patch: &[144, 144] },
    ShadowRegion { offset: 25437811, patch: &[72, 139, 207, 232, 5, 151, 0, 0, 131, 191, 104, 1, 0, 0, 0] },
    ShadowRegion { offset: 19498155, patch: &[137, 81, 84, 72, 139, 73, 24, 232, 249, 156, 70, 0, 72, 133, 255, 116] },
    ShadowRegion { offset: 17308544, patch: &[64] },
    ShadowRegion { offset: 27997915, patch: &[51, 246, 69, 133, 192, 72, 137, 124, 36, 80, 65, 15, 73, 240, 137, 112, 124] },
    ShadowRegion { offset: 7571192, patch: &[116, 33] },
    ShadowRegion { offset: 1068652, patch: &[4] },
];
pub static SR_Y6S1_15680437: &[ShadowRegion] = &[
    ShadowRegion { offset: 69247251, patch: &[233, 237, 0, 0, 0, 144] },
];
pub static SR_Y7S3_47165842: &[ShadowRegion] = &[
    ShadowRegion { offset: 71379443, patch: &[233, 227, 1, 0, 0, 144] },
    ShadowRegion { offset: 85476081, patch: &[233, 58, 3, 0, 0, 144] },
    ShadowRegion { offset: 71288451, patch: &[233, 236, 1, 0, 0, 144] },
    ShadowRegion { offset: 71337443, patch: &[233, 236, 1, 0, 0, 144] },
    ShadowRegion { offset: 71568131, patch: &[233, 239, 1, 0, 0, 144] },
];
pub static SHADOW_SEASONS: &[ShadowSeason] = &[
    ShadowSeason { build: "Y6S2_15701375", regions: SR_Y6S2_15701375 },
    ShadowSeason { build: "Y7S2_43489433", regions: SR_Y7S2_43489433 },
    ShadowSeason { build: "Y7S4_50497889", regions: SR_Y7S4_50497889 },
    ShadowSeason { build: "Y7S1_39785137", regions: SR_Y7S1_39785137 },
    ShadowSeason { build: "Y6S3_32627923", regions: SR_Y6S3_32627923 },
    ShadowSeason { build: "Y6S4_37046065", regions: SR_Y6S4_37046065 },
    ShadowSeason { build: "Y6S1_15500403", regions: SR_Y6S1_15500403 },
    ShadowSeason { build: "Y6S2_15772114", regions: SR_Y6S2_15772114 },
    ShadowSeason { build: "Y6S3_35130040", regions: SR_Y6S3_35130040 },
    ShadowSeason { build: "Y5S1_14303219", regions: SR_Y5S1_14303219 },
    ShadowSeason { build: "Y6S1_15680437", regions: SR_Y6S1_15680437 },
    ShadowSeason { build: "Y7S3_47165842", regions: SR_Y7S3_47165842 },
];
