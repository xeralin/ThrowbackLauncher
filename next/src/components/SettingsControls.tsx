...
export function LevelStepper({
  ...
  const step = (delta: number, refocus: boolean) => {
    ...
    onCommit(levels[next]);
    if (!refocus) return;
    if (next <= 0) increase.current?.focus();
    else if (next >= levels.length - 1) decrease.current?.focus();
  };

  return (
    <div className="flex h-8 items-center rounded-md border border-border bg-surface-2">
      <button
        ref={decrease}
        type="button"
